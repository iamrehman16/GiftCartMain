...
public class Additem {
    public static void main(String[] args) {
        ...
        // Create main frame
        JFrame frame = new JFrame("Gift Registry System");
        ...
        for (String[] item : items) {
            ...
            catalogPanel.add(Box.createVerticalStrut(10));
        }
        ...
        viewRegistryButton.addActionListener(e -> {
            ...
            controller.getAllItems().forEach((name, details) ->
                    registryModel.addElement(name + " - Price: $" + details[0] + " x " + details[1])
            );
            ...
            updateButton.addActionListener(event -> {
                ...
                if (selectedValue != null) {
                    ...
                    String newQty = JOptionPane.showInputDialog(registryFrame, "Enter new quantity for " + itemName + ":");
                    try {
                        int qty = Integer.parseInt(newQty);
                        if (controller.updateItem(itemName, qty)) {
                            registryModel.setElementAt(itemName + " - Price: $" + itemDetails[0] + " x " + qty, registryList.getSelectedIndex());
                            JOptionPane.showMessageDialog(registryFrame, "Item updated successfully.");
                        } else {
                            JOptionPane.showMessageDialog(registryFrame, "Invalid quantity.", "Error", JOptionPane.ERROR_MESSAGE);
                        }
                    } catch (NumberFormatException ex) {
                        JOptionPane.showMessageDialog(registryFrame, "Invalid quantity.", "Error", JOptionPane.ERROR_MESSAGE);
                    }
                } else {
                    JOptionPane.showMessageDialog(registryFrame, "Please select an item to update.", "Error", JOptionPane.ERROR_MESSAGE);
                }
            });
            ...
            deleteButton.addActionListener(event -> {
                ...
                if (selectedValue != null) {
                    ...
                    if (controller.deleteItem(itemName)) {
                        ...
                        JOptionPane.showMessageDialog(registryFrame, "Error deleting item.", "Error", JOptionPane.ERROR_MESSAGE);
                    }
                } else {
                    JOptionPane.showMessageDialog(registryFrame, "Please select an item to delete.", "Error", JOptionPane.ERROR_MESSAGE);
                }
            });
