...
public class Signup {
    private static final controller signupController = new controller();
    ...
    public static void showSignupDialog(JFrame parentFrame) {
        ...
        signupButton.addActionListener(e -> {
            ...
            String signupResult = signupController.processSignup(email, phone, username, role, confirmationCode);

            // Handle controller's response
            if (signupResult.startsWith("Error:")) {
                JOptionPane.showMessageDialog(signupDialog, signupResult, "Error", JOptionPane.ERROR_MESSAGE);
            } else {
                JOptionPane.showMessageDialog(signupDialog, signupResult, "Success", JOptionPane.INFORMATION_MESSAGE);
                signupDialog.dispose(); // Close the dialog after submission
                parentFrame.dispose();

                // Navigate to respective functionality based on role
